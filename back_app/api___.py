  ...
  if request.method == 'POST':
    ...
    if (int(request.args['feederId'])==-1):
      feeder_fields = {
        ...
        'feederType': request.args['feederType'],
        'timeTable': request.args['timeTable'],
        'capacity': request.args['capacity'],
        'filledInternally': request.args['filledInternally'],
        'filledExternally': request.args['filledExternally']
      }
      ...
      feeder_fields = {
        ...
        'feederType': request.args['feederType'],
        'timeTable': request.args['timeTable'],
        'capacity': request.args['timeTable'],
        'filledInternally': request.args['filledInternally'],
        'filledExternally': request.args['filledExternally'],
        'feederId': request.args['feederId']
      }

      for user in users:
        if (user.getUserId()==userId):
          feeder = user.getFeederById(feeder_fields['feederId'])
          print(feeder)
          if (feeder_fields['userId']==userId):
            feeder.set(feeder_fields)
          else:
            user.removeFeederById(feeder_fields['feederId'])
            for user in users:
              if (user.getUserId()==feeder_fields['feederId']):
                feeder = user.getFeederById(feeder_fields['feederId'])
